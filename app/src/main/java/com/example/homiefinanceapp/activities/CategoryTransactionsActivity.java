...
import com.example.homiefinanceapp.databinding.ActivityCategoryTransactionsBinding;
...
public class CategoryTransactionsActivity extends AppCompatActivity {
    ...
    private ActivityCategoryTransactionsBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityCategoryTransactionsBinding.inflate(getLayoutInflater());
        ...
        selectedCategoryType = getIntent().getStringExtra(EXTRA_CATEGORY_TYPE);
        ...
        binding.tvTitle.setText("Giao dịch - " + (selectedCategoryName != null ? selectedCategoryName : ""));
        ...
        loadTransactionsFor(selectedCategoryName, selectedCategoryType, null);
    }
                ...
                .enqueue(new Callback<ApiResponse<String>>() {
                    ...
                    public void onResponse(Call<ApiResponse<String>> call, Response<ApiResponse<String>> response) {
                        if (response.isSuccessful()) {
                            Toast.makeText(
                                    CategoryTransactionsActivity.this,
                                    resolveBeMessage(response, "Xóa giao dịch thành công!"),
                                    Toast.LENGTH_SHORT
                            ).show();
                            refreshTransactionsImmediately();
                        } else {
                            Toast.makeText(
                                    CategoryTransactionsActivity.this,
                    ...
                    public void onFailure(Call<ApiResponse<String>> call, Throwable t) {
                        ...
                        Toast.makeText(CategoryTransactionsActivity.this, "Lỗi kết nối Server!", Toast.LENGTH_SHORT).show();
                    }
                });
    }
                ...
                .enqueue(new Callback<ApiResponse<TransactionListItem>>() {
                    ...
                    public void onResponse(Call<ApiResponse<TransactionListItem>> call, Response<ApiResponse<TransactionListItem>> response) {
                        if (response.isSuccessful() && response.body() != null) {
                            ...
                            Toast.makeText(CategoryTransactionsActivity.this, msg, Toast.LENGTH_SHORT).show();
                            TransactionListItem data = response.body().getData();
                            if (data != null) {
                                replaceTransactionInCurrentList(sourceItem.getId(), data);
                            } else {
    ...
    private void syncReceiptUrlToCurrentList(String transactionId, String receiptUrl) {
    ...
    private void replaceTransactionInCurrentList(String transactionId, TransactionListItem newData) {
    ...
    private void refreshTransactionsImmediately() {
        String keyword = binding.etSearchNote.getText() != null
                ? binding.etSearchNote.getText().toString().trim()
                : "";
        loadTransactionsFor(selectedCategoryName, selectedCategoryType, keyword);
    }
    ...
    private void loadTransactionsFor(String categoryName, String categoryType, String keyword) {
        if (token == null || token.trim().isEmpty()) {
            Toast.makeText(this, "Chưa có token!", Toast.LENGTH_SHORT).show();
            return;
        }
        if (categoryName == null || categoryName.trim().isEmpty()) return;
        if (categoryType == null || categoryType.trim().isEmpty()) return;

        int page = 0;
        int size = 5;

        ApiService apiService = RetrofitClient.getClient().create(ApiService.class);
        Call<ApiResponse<TransactionsPageData>> requestCall;
        String trimmedKeyword = keyword != null ? keyword.trim() : "";
        boolean isSearch = !trimmedKeyword.isEmpty();

        if (isSearch) {
            requestCall = apiService.searchTransactions("Bearer " + token, trimmedKeyword, page, size);
        } else {
            requestCall = apiService.getTransactions("Bearer " + token, page, size);
        }

        requestCall.enqueue(new Callback<ApiResponse<TransactionsPageData>>() {
                    @Override
                    public void onResponse(Call<ApiResponse<TransactionsPageData>> call, Response<ApiResponse<TransactionsPageData>> response) {
                        if (response.isSuccessful()
                            ...
                            for (TransactionListItem item : response.body().getData().getContent()) {
                                if (item == null) continue;
                                boolean nameMatch = item.getCategoryName() != null
                                        && item.getCategoryName().equalsIgnoreCase(categoryName);
                                boolean typeMatch = item.getCategoryType() != null
                                        && item.getCategoryType().equalsIgnoreCase(categoryType);
                            ...
                            Toast.makeText(CategoryTransactionsActivity.this, "Không lấy được transactions!", Toast.LENGTH_SHORT).show();
                        }
                    }

                    @Override
                    public void onFailure(Call<ApiResponse<TransactionsPageData>> call, Throwable t) {
                        Log.e("API_ERROR", "loadTransactionsFor: " + t.getMessage());
                        Toast.makeText(CategoryTransactionsActivity.this, "Lỗi kết nối Server!", Toast.LENGTH_SHORT).show();
                    }
                });
    }
    ...
    public static Intent newIntent(android.content.Context context, Category category) {
        Intent i = new Intent(context, CategoryTransactionsActivity.class);
        if (category != null) {
            i.putExtra(EXTRA_CATEGORY_NAME, category.getName());
            i.putExtra(EXTRA_CATEGORY_TYPE, category.getType());
        }
        return i;
    }
}
